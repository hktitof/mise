...
This is the generated bin directory for `[tool_stubs]` declarations in normal
user config files such as `~/.config/mise/config.toml`. `mise activate` adds it
to `PATH` ahead of the system tool-stub bin. The files are managed output;
customize commands in config rather than editing generated stubs.
...
live under `installs`, while commands generated from `[tool_stubs]` declarations
in normal system config such as `/etc/mise/config.toml` live under
`tool-stubs/bin`. System files are lower-precedence defaults: user tool stubs
with the same command name override them.
