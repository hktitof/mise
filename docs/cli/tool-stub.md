...
- **Usage:** `mise tool-stub [FILE] [ARGS]… <SUBCOMMAND>`
- **Source code:** [`src/cli/tool_stub/mod.rs`](https://github.com/jdx/mise/blob/main/src/cli/tool_stub/mod.rs)
...
The stub will automatically install the specified tool version if missing and execute it with any arguments passed to the stub. Management subcommands synchronize the `[tool_stubs]` catalogues declared in user and system config.
...
- [`mise tool-stub remove [FLAGS] [MANIFEST]`](/cli/tool-stub/remove.md)
